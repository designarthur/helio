...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 font-sans min-h-screen flex items-center justify-center py-8">
    ...
    <div class="bg-white p-8 rounded-lg shadow-xl w-11/12 max-w-3xl relative max-h-[90vh] overflow-y-auto">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <p><strong>Linked Booking:</strong>
                        @if($invoice->linkedBooking)
                            <a href="{{ route('customer.bookings.show', $invoice->linkedBooking->id) }}" class="text-blue-600 hover:underline">#{{ $invoice->linkedBooking->id }}</a>
                        @else
                            N/A
                        @endif
                    </p>
                    <p><strong>Linked Quote:</strong>
                        @if($invoice->linkedQuote)
                            <a href="{{ route('customer.quotes.show', $invoice->linkedQuote->id) }}" class="text-blue-600 hover:underline">#{{ $invoice->linkedQuote->id }}</a>
                        @else
                            N/A
                        @endif
                    </p>
        ...
        <div class="mt-6 flex justify-end gap-3">
            <a href="{{ route('customer.invoices.index') }}" class="px-6 py-2 bg-gray-300 text-gray-800 rounded-md font-semibold hover:bg-gray-400 transition-colors duration-200">Close</a>
            @if($invoice->balance_due > 0)
                <a href="{{ route('customer.payment_methods.index', ['invoice_id' => $invoice->id, 'amount' => $invoice->balance_due]) }}" class="px-6 py-2 bg-chili-red text-white rounded-md font-semibold hover:bg-tangelo transition-colors duration-200">Pay Now</a>
            @endif
            <button onclick="showConceptualAction('Download Invoice', 'Downloading invoice PDF...')" class="px-6 py-2 bg-blue-600 text-white rounded-md font-semibold hover:bg-blue-700 transition-colors duration-200">Download PDF</button>
        </div>
    </div>
</body>
</html>