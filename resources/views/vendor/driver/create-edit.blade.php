...
@section('content')
    <h2 class="text-3xl font-bold text-gray-900 mb-6">{{ isset($driver) ? 'Edit Driver' : 'Add New Driver' }}</h2>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        <form action="{{ isset($driver) ? route('drivers.update', $driver->id) : route('drivers.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="mb-4">
                    <label for="license_number" class="block text-sm font-medium text-gray-700 mb-1">Driver's License Number</label>
            ...
            <div class="mb-4">
                <label for="vehicle_id" class="block text-sm font-medium text-gray-700 mb-1">Assigned Vehicle (optional)</label>
            ...
            <div class="mb-4">
                ...
                <select name="status" id="status"
                    ...
                    <option value="On Duty" {{ (old('status', $driver->status ?? '') == 'On Duty') ? 'selected' : '' }}>On Duty</option>
                    <option value="Off Duty" {{ (old('status', $driver->status ?? '') == 'Off Duty') ? 'selected' : '' }}>Off Duty</option>
                    <option value="Incapacitated" {{ (old('status', $driver->status ?? '') == 'Incapacitated') ? 'selected' : '' }}>Incapacitated</option>
                </select>
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="notes" id="notes" rows="3"
                          class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-chili-red focus:border-chili-red sm:text-sm"
                          placeholder="Any additional notes about the driver">{{ old('notes', $driver->notes ?? '') }}</textarea>
            </div>
