...
@section('content')
    <h2 class="text-3xl font-bold text-gray-900 mb-6">User Details: {{ $user->name }}</h2>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md mb-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <p class="text-lg text-gray-900">{{ $user->last_login_at ? $user->last_login_at->format('M d, Y H:i A') : 'Never' }}</p>
            </div>
            <div>
        ...
        <div class="mt-8 flex justify-end gap-3">
            <a href="{{ route('settings.users.edit', $user->id) }}" class="px-6 py-3 bg-blue-600 text-white rounded-md font-semibold hover:bg-blue-700 transition-colors duration-200">
                Edit User
            </a>
            <form action="{{ route('settings.users.destroy', $user->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this user? This action cannot be undone.');">
                @csrf
                @method('DELETE')
